...
function getCSS(files, pages, stylesheets, options, callback) {
    ...
    return lib.readStylesheets(stylesheets, function (err, stylesheetsWithFileName) {
  	  return callback(err, pages, stylesheetsWithFileName, options);
    });
}
 ...
 * @param  {Array}    stylesheetsWithFileName list of CSS files
 * @param  {Object}   options     options passed to uncss
 * @param  {Function} callback(Error, String, Object)
 
 */
 //for unused css report
function uncss(pages, stylesheetsWithFileName, options, callback) {
    /* If we specified a raw string of CSS, add it to the stylesheets array */
    var styleObjWithFilepath = {};
    if (options.raw) {
        if (typeof options.raw === 'string') {
            styleObjWithFilepath.filepath = 'inline_CSS';//changes according to new Array stylesheetsWithFileName
            styleObjWithFilepath.stylesheet = options.raw;
            stylesheetsWithFileName.push(styleObjWithFilepath);
        } else {
     ...
     */
    //for unused css report
    var fileObjLen = stylesheetsWithFileName.length,
    parsed,
    report,
    parsedWithFileNameArr = [];
    ...
    for(var i = 0; i < fileObjLen; i++){
        // Modified according to new new Array stylesheetsWithFileName
        
        if (_.flatten(stylesheetsWithFileName[i].stylesheet).length === 0) {
            return callback(new Error('UnCSS: no stylesheets found'));
        }else{
            try{

                var parsedAndFile = {};
                parsed = css.parse(stylesheetsWithFileName[i].stylesheet);
                parsedAndFile.parsed = parsed;
                parsedAndFile.filepath = stylesheetsWithFileName[i].filepath;
                parsedWithFileNameArr.push(parsedAndFile);
            }
            catch (err) {
                ...
                err.message = 'uncss/node_modules/css: ' + err.message+" file origin:"+styleObjWithFilepath[i].filepath;
                return callback(err);
            }
        }
    }
    ...
    //for unused css report
    return lib.uncss(pages, parsedWithFileNameArr, options.ignore, function (unused_css) {
    var ususedCssLen  = unused_css.length,
    	fileData = '';
    for (var i = 0; i < ususedCssLen; i++){
        for (var count in unused_css[i].processed ){
		if(unused_css[i].processed[count].type === 'rule'){
			fileData += unused_css[i].filepath+ '  -Rules-> ' + unused_css[i].processed[count].selectors[0]+'\n';
            	}else if (unused_css[i].processed[count].type === 'media'){
                	for(var index in unused_css[i].processed[count].rules){
                 	       fileData += unused_css[i].filepath+ '  -Media-> ' + unused_css[i].processed[count].rules[index].selectors[0]+'\n';
                	}
		}
	}
    }
