        ...
        if (filename.match(/^http/)) {
            ...
            if (fs.existsSync(filename)) {
                ...
                return fs.readFile(filename, 'utf8', function(err,data){
                   var styleObj = {};
                   styleObj.filepath = filename ; 
                   styleObj.stylesheet = data;
                   done(err,styleObj);
                });
...
function filterUnusedSelectors(pages, selectors, ignore, used_selectors) {
    ...
    return selectors.filter(function (selector) {
        ...
        //for unused css report: getting selector which are not used
 ...
 * @param {String}   filename	    CSS file name which is getting parsed
 * @param {Function} callback(Error, Array)
 * @param {Boolean}  isRec          Used internally
 */
function getUsedSelectors(page, stylesheet, filename, callback, isRec) {
    
    return async.concat(
        stylesheet.rules,
        function (rule, done) {
            if (rule.type === 'rule') {
            	return done(null, [rule.selectors,filename]);
            } else if (rule.type === 'media') {
                return getUsedSelectors(page, rule, filename,done, true);
            }
            return done(null, []);
        },
        function (err, selectors) {
            ...
            return phantom.findAll(page, selectors[0].map(dePseudify), callback,filename);
        }
    );
}
 ...
 * @param {Array}    pages      list of PhantomJS pages
 * @param {Array}    parsedWithFileNameArr list of all selectors with filename
 * @param {Object}   stylesheet The output of css.parse().stylesheet
 * @param {Array}    ignore     List of selectors to be ignored
 * @param {Function} callback(Error, CSS)
 */
function uncss(pages, parsedWithFileNameArr, ignore, callback) {
    return async.concat(
        pages,
        ...
        function (page, done) {
            ...
            async.concat(
                ...
                function (element, innerCallback){
                    return getUsedSelectors(page, element.parsed.stylesheet,element.filepath, innerCallback);
                },
                function (err, used_object){
                    if (err) {
                       
                        return callback(err);
                    }else{
                       
                       return done (err, used_object) ; 
                    }
        ...
        function (err, used_selectors) {
            ...
            var usedLen = used_selectors.length,
            	usedSelectorsArr = [];
            for (var i = 0; i < usedLen; i++){
                if(typeof(used_selectors[i]) !== 'string' ) {
                    usedSelectorsArr.push(used_selectors[i].join());
                }
         }
            ...
            var processed = '',
                unusedRulesArr = [];
           // filtering unused Rules     
            parsedWithFileNameArr.forEach(function (element) {
                processed = filterUnusedRules(pages, element.parsed.stylesheet, ignore, usedSelectorsArr);
                unusedRulesArr.push({processed:processed,filepath:element.filepath});
            });
            return callback(unusedRulesArr);
        }
    );
}
